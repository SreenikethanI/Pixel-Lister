...
/** Error distribution function for Floyd-Steinberg dithering
 * @param {ImageData} image
 * @param {number} errorR the `old - new` value for the R channel
 * @param {number} errorG the `old - new` value for the G channel
 * @param {number} errorB the `old - new` value for the B channel
...
function ditherErrorDistGrayscaleTemplate(image, nBits, errorDistFn, gamma=2.2) {
    ...
    const w = image.width, h = image.height;
    const linearMap = generateLinearMap(gamma);
    const quantFactor = 256 / (2 ** parseInt(nBits)); // cache value
    ...
    for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
            ...
            const gray = 255 * ((0.2126 * cRLin + 0.7152 * cGLin + 0.0722 * cBLin) ** 0.5);
            const newColor = Math.round(gray / quantFactor) * quantFactor;
            result[index+1] = newColor;
            result[index+2] = newColor;
            result[index+3] = newColor;
            errorDistFn(image, cR - newColor, cG - newColor, cB - newColor, x, y);
        }
    }
    return result;
}
...
export function ditherFS(image, palette, gamma=2.2) {
    ditherErrorDistTemplate(image, palette, FSErrorFn, gamma);
}

export function ditherFSGrayscale(image, nBits, gamma=2.2) {
